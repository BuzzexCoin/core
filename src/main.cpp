...
#if defined(NDEBUG)
#error "BuzzexCoin cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in ubzx) are considered zero fee (for relaying and mining)
...
bool CheckTransaction(const CTransaction& tx, CValidationState& state)
{
    ...
    if (tx.IsCoinBase()) {
        if ((tx.vin[0].scriptSig.size() < 2 || tx.vin[0].scriptSig.size() > 150) && !Params().SkipProofOfWorkCheck())
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
    ...
    ret = blockValue * 0.60; // 60% of block reward
...
{
    RenameThread("buzzexcoin-scriptch");
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // BZX: We use certain sporks during IBD, so check to see if they are
