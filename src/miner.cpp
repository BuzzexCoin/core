...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
    ...
    pblock->nVersion = 4;
	if (!fProofOfStake) pblock->nVersion = 1;
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("BuzzexCoinMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("BuzzexCoinMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("BuzzexCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("buzzexcoin-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running BuzzexCoinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
