...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a BZX address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
{
    // return if URI is not valid or is no buzzexcoin: URI
...
{
    // Convert buzzexcoin:// to buzzexcoin:
    //
    //    Cannot handle this later, because buzzexcoin:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open buzzexcoin.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "BuzzexCoin.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for BuzzexCoin.lnk
...
{
    return GetAutostartDir() / "buzzexcoin.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a buzzexcoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=BuzzexCoin\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the buzzexcoin app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add buzzexcoin app to startup item list
